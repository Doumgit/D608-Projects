---
title: "Souleymane D. Data608-Story 4"
...
ggplot(role_overview, aes(x = reorder(Classification, Average_Salary), y = Average_Salary)) +
...
```{r Salary_by_Role_National_Level_2nd}
...
```{r Salary_by_Role_by_Industry}
#-----------------------------------------------------------------------
## Data overview: summarizing total employment and mean salary by role
industries_salary <- filtered_data_cleaned %>%
  filter(AREA_TYPE == '1', OWN_CODE != '1235', Broad_Industry != "Government (Federal, State, Local)" ) %>%
...
# Bar chart of average salary by role
ggplot(industries_salary, aes(x = reorder(Broad_Industry, Average_Salary), y = Average_Salary)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  ...
  theme_minimal()
```
# Data Practioner Average Salary by Industry (National Level) 2nd 
```{r Salary_by_Role_by_Industry_2nd}
...
ggplot(industries_salary, aes(x = reorder(Broad_Industry, Average_Salary), y = Average_Salary, fill = Color)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = 113268.18, color = "lightblue", linetype = "dashed", size = 0.7) +  # Add red dashed line at $110,000
  ...
  geom_hline(yintercept = 115394, color = "steelblue", linetype = "dashed", size = 0.7) +
...
# Top 10 Private Sector Paying high Data Practioner Average Salary
```{r Salary_by_Role_Private_Sector}
#-----------------------------------------------------------------------
## Data overview: summarizing total employment and mean salary by role
...
# Bar chart of average salary by role
ggplot(industries_private, aes(x = reorder(NAICS_TITLE, Average_Salary), y = Average_Salary)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(title = "Top 10 High Pay Private Sectors For Data Practioner Job",
       x = "Pivate Sector",
       y = "Average Salary (USD)") +
  theme_minimal()
```
...
ggplot(industries_private, aes(x = reorder(NAICS_TITLE, Average_Salary), y = Average_Salary, fill = Color)) +
  ...
  geom_hline(yintercept = 209000, color = "steelblue", linetype = "dashed", size = 0.9) +  # Add red dashed line at $200,000
  annotate("text", x = "Hydropower", y = 209000, label = "$210,000", vjust = -.5, hjust = 0.5, color = "steelblue", size = 4.5, fontface = "bold", angle = 90) +
  
  geom_hline(yintercept = 167467.5, color = "green", linetype = "dashed", size = 0.9) +  # Add red dashed line at $200,000
...
# Salary by Role (State Level)
```{r Salary_by_Role_State_Level}
#-----------------------------------------------------------------------
## Data overview: summarizing total employment and mean salary by role
suppressMessages(
...
ggplot(role_state_overview, aes(x = AREA_TITLE, y = Classification, fill = Average_Salary)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 10),
        ...
        plot.margin = unit(c(1, 1, 1, 1), "cm")  # Add spacing around the plot
  )
#-----------------------------------------------------------------------
```
...
# Prepare your data for merging by converting state names to lowercase
role_state_overview <- role_state_overview %>%
  mutate(AREA_TITLE = tolower(AREA_TITLE)) # Convert state names to lowercase for merging

# Merge US map data with your summarized salary data
suppressWarnings({
  merged_data <- us_map %>%
  left_join(role_state_overview, by = c("region" = "AREA_TITLE"))
})

# Plot: Heatmap on the US map faceted by Data Practitioner Role
ggplot(merged_data, aes(long, lat, group = group, fill = Average_Salary)) +
  geom_polygon(color = "white") +
  coord_fixed(1.3) +  # Ensures the map proportions are accurate
  scale_fill_gradient(low = "lightblue", high = "darkblue", na.value = "grey", 
                      name = "Average Salary (USD)") +
  labs(title = "Average Salary by State and Data Practitioner Role",
       x = "Longitude",
       y = "Latitude") +
  facet_wrap(~ Classification) +  # Creates a separate map for each role
  theme_minimal() +
